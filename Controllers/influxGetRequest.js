...
const tagsParams = {
    ...
    "telemetry": {
        ...
        "sps30": { "pm1.0": 1.1, "pm2.5": 1.1, "pm4.0": 1.1, "pm10.0": 1.1, "nc0.5": 1.1, "nc0.5": 1.1, "nc1.0": 1.1, "nc2.5": 1.1, "nc4.0": 1.1, "nc10.0": 1.1, "typical_size": 0.5 },
...
exports.getDataFromMySql = (req, res) => {
    ...
    const coops = "SELECT farm.ID as `farmId`, `Farm_Name`, coops.ID as `coopsId`, `Name`, `position`, `Device_ID` FROM `farm`, `coops`, `sensors` WHERE farm.ID = '" +
        req.body.id + "' AND '" + req.body.id + "' = coops.Farm_ID AND coops.ID = sensors.Coops_ID";
        ...
        if (error) {
            ...
            results.forEach((item) => {
                let point = new Point(item.Farm_Name + item.farmId)
                    ...
                    .stringField('sps30', JSON.stringify('{ "pm1.0": 1.1, "pm2.5": 1.1, "pm4.0": 1.1, "pm10.0": 1.1, "nc0.5": 1.1, "nc0.5": 1.1, "nc1.0": 1.1, "nc2.5": 1.1, "nc4.0": 1.1, "nc10.0": 1.1, "typical_size": 0.5 }'))
